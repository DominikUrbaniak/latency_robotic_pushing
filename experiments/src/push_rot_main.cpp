...
std::string dir_path = "docs/data/rot/";
  ...
  private:
    ...
    void timer_callback()
    {
      ...
      if(get_model_state_euler_delayed.yaw > cube_yaw_goal){ //the condition >= 0 triggers at the beginning when no measurements are available. Here the state switches at -PI to +PI, hence >0 is sufficient
        ...
        history.push_back({seconds_eef,tcp_pose[0],tcp_pose[1],tcp_pose[2],tcp_pose[5],tcp_x_,tcp_y_,(float)trigger,seconds_pose_sensed,seconds_pose_delayed,(float)latency_ms,get_model_state_pose_delayed.position.x,get_model_state_pose_delayed.position.y,get_model_state_euler_delayed.yaw});
...
int main(int argc, char * argv[])
{
  ...
  std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("push_rot_main");
  srand ( time(NULL) );
