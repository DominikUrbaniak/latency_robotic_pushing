...
bool freshest = false;
std::string network_filename = dir_path + "125Hz_60Hz/latencies/" + "1683300855_latencies";
...
int main(int argc, char * argv[])
{
  ...
  if(set_delay_mode==1 || set_delay_mode==2){
    //read latency data
    if(network.size()>2){
      network_filename = dir_path + network.c_str();
    }
    //std::string network_filename = network_directory + network.c_str() + ".csv";
    RCLCPP_INFO(node->get_logger(), "Replay data file name: %s", network_filename.c_str());
    std::ifstream  data(network_filename);
    if(data.is_open())
    {
      std::string line;
      while(std::getline(data, line)){
        latency_vector.emplace_back(std::stold(line));
      }
    }
    latency_distribution_size = (int)latency_vector.size();
    if(latency_distribution_size==0){
      RCLCPP_ERROR(node->get_logger(), "Latency replay data not found at %s", network_filename.c_str());
      return -1;
    }
    ...
    if(freshest){
      ...
      for(int i=0;i<1000;i++){
        ...
        for(int j = i+1;j<1000;j++){
          ...
          if(j-i>5){//check for the next 5 latencies
            break;
          }
          else{
  ...
  if(save_params_to_csv){
    ...
    if(set_delay_mode==1){
      filename = dir_path + std::to_string(secs) + "_" + std::to_string(set_delay_mode) + "mode_"+ std::to_string(control_rate_ms)+ "ms_" + std::to_string(frame_rate_microsec)+ "microsecs_" + std::to_string(freshest) +"_"+ std::to_string(step_size) + "steps.csv";
    }
    else if(set_delay_mode==3){
      ...
      filename = dir_path + std::to_string(secs) + "_" + std::to_string(set_delay_mode) + "mode_"+ std::to_string(control_rate_ms)+ "ms_" + std::to_string(frame_rate_microsec)+ "microsecs_" + std::to_string(computation_latency/1000)+"cl_"+ std::to_string(freshest) +"_"+ std::to_string(step_size) + "steps.csv";
    }
    else{
    ...
    //RCLCPP_INFO(node->get_logger(), "Open file name: %s", filename.c_str());
  ...
  /*if(latency_vector.size() > latency_vector_fresh.size()){
    filename_latencies = dir_path + std::to_string(secs) + "_latencies";
